...
def get_statistical_features(v, field_type, field_general_type):
    ...
    if field_general_type in 'q':
        ...
        """if np.any(v > 0):
            positive_v = v[v > 0]
            entropy_val = entropy(positive_v)
        else:
            entropy_val = 0.0 """ 
        ...
        # Check for inf
        #if math.isinf(entropy_val): #and entropy_val < 0:
        #    entropy_val = -np.finfo(float).eps

        # Use the entropy value in further calculations
        #r['entropy'] = entropy_val
        ...
        #denominator = np.where((q75 + q25) == 0, 1e-16, (q75 + q25)) #check for non zeros
        #r['quant_coeff_disp'] = (q75 - q25) / denominator
        #r['quant_coeff_disp'] = (q75 - q25) / (q75 + q25)
        #print("quant_coeff_disp",r['quant_coeff_disp'])
...
def get_sequence_features(v, field_type, field_general_type):
    ...
    if field_general_type == 'q':
        ...
        #sorted_v = np.sort(v)
        #divisor = sorted_v[1:]
        #divisor = np.where(divisor == 0, 1e-16, divisor) # replace any zeros with a very small non-zero value
        #sequence_incremental_division = np.divide(sorted_v[:-1], divisor)
        #sequence_incremental_division = np.divide(sorted_v[:-1], sorted_v[1:]) # removing this feature for now
        #print("sequence_incremental_division", sequence_incremental_division)
       ...
       # print("log_space_sequence_coeff", r['log_space_sequence_coeff'])
        r['is_lin_space'] = r['lin_space_sequence_coeff'] <= 0.001
        
        #print(r['is_lin_space'])
    ...
    for i, (field_name, d) in enumerate(fields):
        ...
        try:
            ...
            #find longest element
                # Check if field_type is integer or float
            if field_type == "integer" or field_type == "float" or field_type == "decimal":
                max_len = 0
                for j in range(len(v)):
                    if len(str(v[j])) > max_len:
                        max_len = len(str(v[j]))
                if max_len > 6:
                    # Create a StandardScaler object
                    #scaler = StandardScaler()
                    #scaler = MinMaxScaler()
                     # Fit and transform the array values
                    scaler = MinMaxScaler(feature_range=(0.1, 1))
                    v_scaled = scaler.fit_transform(v.reshape(-1, 1))

                    # Update the original array with the scaled values
                    v = np.ravel(v_scaled)

                #print(v)

            if field_name in ("Longitude", "LON", "LONGITUDE"):
                Max_Range = 180
                Min_Range = -180 
                scaler = MinMaxScaler(feature_range=(0.1, 1))
                # Clip values outside the range [Min_Range, Max_Range]
                v_clipped = np.clip(v, Min_Range, Max_Range)
                # Scale the clipped values to the range [0, 1]
                v_scaled = scaler.fit_transform(v_clipped.reshape(-1, 1))
                # Convert the scaled array back to a 1D array
                v = np.ravel(v_scaled)
                #v = scaler.fit_transform(v.reshape(-1, 1))
                #print(v)

            if field_name in ("Latitude", "LAT", "LATITUDE"):
                Max_Range = 90
                Min_Range = -90
