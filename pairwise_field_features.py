...
from helpers import get_unique, get_list_uniqueness, get_shared_elements, calculate_overlap
...
def get_general_pairwise_features(a, b):
    r = OrderedDict([(f['name'], None)
                    for f in general_pairwise_features_list])
    a_data = a['data']
    b_data = b['data']
    ...
    r['percent_shared_unique_elements'] = num_shared_unique_elements / \
        max(len(a_unique_data), len(b_unique_data))
...
def get_statistical_pairwise_features(a, b, MAX_GROUPS=50):
    r = OrderedDict([(f['name'], None)
                    for f in statistical_pairwise_features_list])
    a_name = a['name']
    b_name = b['name']
    a_data = a['data']
    b_data = b['data']
    ...
    if (a['general_type'] == 'c' and b['general_type'] == 'c'):
        ...
        for parent, child in ([a, b], [b, a]):
            ...
            nestedness = get_list_uniqueness(
                child_field_unique_corresponding_values)
    ...
    if (a['general_type'] == 'q' and b['general_type'] == 'c') or (
            a['general_type'] == 'c' and b['general_type'] == 'q'):
        c_field = a
        q_field = b
        ...
        if len(unique_c_field_values) <= MAX_GROUPS:
            df = pd.DataFrame({a_name: a_data, b_name: b_data})
            group_values = [df[df[c_field['name']] == v]
                            [q_field['name']] for v in unique_c_field_values]
        ...
        for field_2_index in range(0, num_fields - field_1_index - 1):
            ...
            try:
                ...
                while time() < (start_time + timeout):
                    name_pairwise_features = get_name_pairwise_features(
                        a['name'], b['name'])
                    statistical_pairwise_features = get_statistical_pairwise_features(
                        a, b)
                    general_pairwise_features = get_general_pairwise_features(
                        a, b)
                    break
            except Exception as e:
                print('Error getting features for {} and {}'.format(
                    a['name'], b['name']))
                pass
